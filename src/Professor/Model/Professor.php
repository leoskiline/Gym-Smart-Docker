...
class Professor
{
     ...
     * @param $dataDemissao
     */
    public function __construct($idProfessor, $nome, $salario, $dataNascimento, $sexo, $estadoCivil, $rua, $nrcasa, $bairro, $cidade, $uf, $pais, $cep, $contato, $email, $rg, $cpf, $dataAdmissao, $dataDemissao)
    {
    ...
    public function obterProfessorById(?\PDO $conn)
    {
        $stmt = $conn->prepare("SELECT * FROM feminina.professor WHERE idProfessor = :idProfessor");
    ...
    public function obterTodosProfessores(?\PDO $conn)
    {
        try{
            $stmt = $conn->prepare("SELECT idProfessor,nome,salario,DATE_FORMAT(dataNascimento, '%d/%m/%Y') as dataNascimento,sexo,estadoCivil,rua,nrcasa,bairro,cidade,uf,pais,cep,contato,email,rg,cpf,DATE_FORMAT(dataAdmissao, '%d/%m/%Y') as dataAdmissao,dataDemissao FROM feminina.professor");
            $stmt->execute();
    ...
    public function obterProfessorPorCPF(?\PDO $conn)
    {
        $stmt = $conn->prepare("SELECT * FROM feminina.professor WHERE cpf = :cpf");
    ...
    public function gravarProfessor(?\PDO $conn)
    {
        ...
        $stmt->execute();
        $this->setIdProfessor($conn->lastInsertId());
    }

    public function gravarUsuario(?\PDO $conn, bool|string $senha)
    {
        $stmt = $conn->prepare("INSERT INTO feminina.usuario (nivel,email,senha,dataCadastro,usuarioAtivo,idProfessor) VALUES (:nivel,:email,:senha,:dataCadastro,:usuarioAtivo,:idProfessor)");
        $stmt->bindValue(":nivel","Professor",\PDO::PARAM_STR);
        $stmt->bindValue(":email",$this->getEmail(),\PDO::PARAM_STR);
    ...
    {
        $stmt = $conn->prepare("UPDATE feminina.usuario SET email = :email, senha = :senha WHERE idProfessor = :idProfessor");
        $stmt->bindValue(":email",$this->getEmail(),\PDO::PARAM_STR);
        $stmt->bindValue(":senha",$senha,\PDO::PARAM_STR);
        $stmt->bindValue(":idProfessor",$this->getIdProfessor(),\PDO::PARAM_INT);
        return $stmt->execute();
    ...
    public function atualizarProfessor(?\PDO $conn)
    {
        $stmt = $conn->prepare("UPDATE feminina.professor SET nome = :nome,dataNascimento = :dataNascimento,sexo = :sexo,estadoCivil = :estadoCivil,
    ...
    {
        try{
            $stmt = $conn->prepare("DELETE FROM feminina.usuario WHERE idProfessor = :idProfessor");
            $stmt->bindValue(":idProfessor",$this->getIdProfessor(),\PDO::PARAM_INT);
            return $stmt->execute();
        }catch (\PDOException $e)
        {
    ...
    {
        try{
            $stmt = $conn->prepare("DELETE FROM feminina.professor WHERE idProfessor = :idProfessor");
            $stmt->bindValue(":idProfessor",$this->getIdProfessor(),\PDO::PARAM_INT);
            return $stmt->execute();
        }catch (\PDOException $e)
        {
    ...
    public function possuiAgendamento(?\PDO $conn)
    {
        try{
            $stmt = $conn->prepare("SELECT * FROM feminina.agendamentosavaliacaofisica WHERE idProfessor = :idProfessor");
            $stmt->bindValue(":idProfessor",$this->getIdProfessor(),\PDO::PARAM_INT);
            $stmt->execute();
